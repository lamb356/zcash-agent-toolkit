...
use crate::types::{MessageType, HEADER_SIZE, MEMO_SIZE, PAYLOAD_SIZE, PROTOCOL_VERSION};
...
pub enum MemoError {
    ...
    /// Payload exceeds the maximum of 458 bytes.
...
/// Structured header occupying the first 54 bytes of every memo.
#[derive(Debug, Clone, PartialEq, Eq)]
...
///
/// The payload must be at most `PAYLOAD_SIZE` (458) bytes. Shorter payloads
...
///
/// For non-binary message types trailing zeros are stripped from the payload.
/// For `MessageType::Binary` the full 458-byte payload is preserved.
pub fn decode_memo(bytes: &[u8; MEMO_SIZE]) -> Result<(MemoHeader, Vec<u8>), MemoError> {
    ...
    let msg_type =
        MessageType::try_from(bytes[1]).map_err(|b| MemoError::InvalidMessageType(b))?;
    ...
    let payload = if msg_type.is_binary() {
        // Preserve full payload for binary types.
        raw_payload.to_vec()
    } else {
        // Strip trailing zeros for text-oriented types.
        let end = raw_payload
            .iter()
            .rposition(|&b| b != 0)
            .map(|i| i + 1)
            .unwrap_or(0);
        raw_payload[..end].to_vec()
    };
...
mod tests {
    ...
    fn roundtrip_encode_decode_all_types() {
        ...
        for mt in types {
            ...
            if mt.is_binary() {
                // Binary preserves full 458-byte payload (zero-padded).
                assert_eq!(dec_payload.len(), PAYLOAD_SIZE);
                assert_eq!(&dec_payload[..payload.len()], payload);
            } else {
